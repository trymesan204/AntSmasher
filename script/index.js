function Ball(x, y, color, dx, dy, index){
    this.x = x;
    this.y = y;
    this.color = color;
    this.dx = dx;
    ...
    this.element.innerHTML = '<img style="width: 50px; height: 50px;" src="./images/ant.png" />';

    this.element.addEventListener('click', function(){
        box.removeChild(this.element);
        console.log(this.element);
        balls[index] = undefined;
    }.bind(this));
}
...
function move(ballObj){
    ...
    setInterval(function(){
        ...
        if (positionX >= 550 || positionX <= 5){
            ballObj.dx = -ballObj.dx;
        }
        if(positionY <= 5 || positionY >= 350){
...
balls.forEach( function(ball, index) {
    ...
    var ballObj = new Ball(ball.x, ball.y, ball.color, dx, dy, index);
...
function calculateDistance(){
    for ( var i = 0; i < allBalls.length; i++ ){
        ...
        for ( var j = i+1; j < allBalls.length; j++){
            ...
            if (dist < 35){
                if(allBalls[i].dx > 0 && allBalls[j].dx > 0){
